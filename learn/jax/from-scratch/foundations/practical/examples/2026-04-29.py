...
def mlp_forward(params, x):
    ...
    """
    l1_res = nnx.relu(jnp.dot(params["W1"], x) + params["b1"])
    l2_res = jnp.dot(params["W2"], l1_res) + params["b2"]
    return l2_res
...
batched_mlp = vmap(mlp_forward, (None, 0))
...
batched_loss = vmap(mse_loss_single, (None, 0, 0))
...
per_example_grad_fn = vmap(grad(mse_loss_single), (None, 0, 0))
...
print(f"Avg W1: {avg_grads['W1'].shape}")
...
def train_step(params, X, Y):
    loss, grads = vmap(jax.value_and_grad(mse_loss_single), (None, 0, 0))(params, X, Y)
    return loss, grads
...
over_spatial = vmap(apply_kernel, (None, 0))
over_batches = vmap(over_spatial, (None, 0))
result = over_batches(kernel, patches)
...
batch_size = 4
Q_batch = random.normal(jax.random.PRNGKey(1), (batch_size, seq_len, head_dim))
K_batch = random.normal(jax.random.PRNGKey(2), (batch_size, seq_len, head_dim))
V_batch = random.normal(jax.random.PRNGKey(3), (batch_size, seq_len, head_dim))

batched_attention = vmap(attention_single)
