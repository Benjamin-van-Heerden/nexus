...
**6.** In Rust, the last expression in a block without a _______ becomes the block's value.

**7.** The idiomatic way to write an infinite loop in Rust is _______, not `while true`.

**8.** To make a range inclusive (include the end value), use _______ instead of `..`.

**9.** In Rust, conditions must be _______ type — there's no truthy/falsy like in Python.
