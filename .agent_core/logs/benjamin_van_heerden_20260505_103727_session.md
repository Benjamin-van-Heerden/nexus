...
### Post-completion fix — OpenClaw execution model

After completing the spec, Benjamin clarified the expected server execution model:

- OpenClaw has long-lived subagents for `manage`, `self`, `learn`, and now `news`.
- Cold/cleared contexts run `onboard`.
- Warm contexts run `refresh`.
- `news` should fit that same model instead of requiring special scheduler logic.

Updated `nexus news refresh` so it is self-routing:

- If today's daily digest exists, refresh behaves as the lightweight breaking/developing-news pass.
- If today's daily digest does not exist, refresh automatically runs the full daily newspaper pipeline, saves today's record, prints the full newspaper, and includes normal agent instructions.

Extracted `_run_full_daily_pipeline(command_name, prefix_message=None)` so `onboard()` and refresh's full-run path share the same implementation.

Also made `XAI_API_KEY` validation lazy:

- Root CLI import no longer fails if `XAI_API_KEY` is missing.
- `_get_xai_client()` now raises `RuntimeError("XAI_API_KEY must be set to use nexus news.")` only when a news command actually tries to use xAI.

Updated `src/commands/news/agent_instructions.md` to document the warm-context behavior: `refresh` is safe to run; if no daily record exists yet, it performs the full daily run.

Verified with:

- `XAI_API_KEY= uv run python` importing the root CLI and checking `manage --help` and `news --help`.
- `uvx ty check env_settings.py src/utils/news.py src/commands/news/onboard.py`.
...
- `src/commands/news/onboard.py` — uses RSS caps from config; passes tracked stories into all sourcing calls; prints source names; shared full daily pipeline used by both onboard and refresh; refresh self-routes to full daily run when today's record is missing.
- `src/commands/news/main.py` — new Typer app wiring for news commands.
- `src/commands/news/agent_instructions.md` — new agent-facing instructions, including refresh self-routing behavior for warm OpenClaw contexts.
...
- `env_settings.py` — made xAI API key validation lazy so unrelated CLI commands do not fail at import time.
...
- Deploy with `XAI_API_KEY` configured on the OpenClaw server; import no longer requires the key, but actual news sourcing still does.
